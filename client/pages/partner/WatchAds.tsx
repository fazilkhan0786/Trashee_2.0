...
import { ArrowLeft, Play, Pause, Clock, Coins, Eye, Gift, CheckCircle } from 'lucide-react';
...
export default function PartnerWatchAds() {
  const navigate = useNavigate();
  const [currentAd, setCurrentAd] = useState<Ad | null>(null);
  const [isPlaying, setIsPlaying] = useState(false);
  const [progress, setProgress] = useState(0);
  const [watchedAds, setWatchedAds] = useState(3);
  const [earnedPoints, setEarnedPoints] = useState(15);
        ...
        <Card className="bg-gradient-to-r from-purple-50 to-blue-50 border-purple-200 animate-in fade-in-50 slide-in-from-bottom-4 duration-700 delay-200">
          <CardContent className="p-4">
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1">
                <h3 className="font-semibold text-purple-800">Partner Bonus</h3>
                <p className="text-sm text-purple-600">Partners can watch up to {maxAdsPerDay} ads daily (5 more than regular users)</p>
              </div>
