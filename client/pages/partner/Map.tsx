...
import { ArrowLeft, Search, MapPin, Navigation, Filter, Store, Trash2, Clock, Zap, Plus, Star } from 'lucide-react';
...
export default function PartnerMap() {
  ...
  const mapMarkers: MapMarker[] = locations
    .filter(location => {
      ...
      if (filterType === 'bin') return location.type === 'bin';
      if (filterType === 'shop') return location.type === 'shop';
      return true;
    })
    .filter(location => {
    ...
    .map(location => ({
      id: location.id,
      lat: 28.6139 + (Math.random() - 0.5) * 0.02, // Random coordinates around ahemdabad
      lng: 77.2090 + (Math.random() - 0.5) * 0.02,
      label: location.name,
      type: location.status === 'active' ? 'success' : 
            location.status === 'full' ? 'error' : 
            location.status === 'inactive' ? 'warning' : 'default',
      ...
      <div className="bg-white border-b border-gray-200 p-4 animate-in slide-in-from-top duration-500">
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1">
            ...
            <p className="text-sm text-gray-500">Find bins and partner shops</p>
          </div>
          <Dialog open={showAddShop} onOpenChange={setShowAddShop}>
            ...
            <DialogContent>
              <DialogHeader>
                <DialogTitle>Add Your Shop Location</DialogTitle>
                <DialogDescription>
                  Add your shop to the map to increase visibility
                </DialogDescription>
              </DialogHeader>
            ...
            <div className="flex gap-3">
              <Select value={filterType} onValueChange={setFilterType}>
                ...
                <SelectContent>
                  ...
                  <SelectItem value="bin">Trash Bins Only</SelectItem>
                  <SelectItem value="shop">Partner Shops Only</SelectItem>
                </SelectContent>
              </Select>
                  ...
                  <div className={`w-12 h-12 rounded-full flex items-center justify-center ${
                    ...
                    {location.type === 'bin' ? (
                      <Trash2 className={`w-6 h-6 ${
                        location.fillLevel && location.fillLevel > 80 ? 'text-red-600' : 'text-green-600'
                      }`} />
                    ) : (
                  ...
                  <div className="flex-1">
                    <div className="flex items-start justify-between mb-2">
                      ...
                      <div className="text-right">
                        ...
                        {location.type === 'shop' && location.rating && (
                    ...
                    <div className="flex justify-between items-center mt-3">
                      <Badge variant={location.status === 'active' ? 'default' : location.status === 'full' ? 'destructive' : 'secondary'}>
                        {location.status}
                      </Badge>
              ...
              {selectedLocation.type === 'bin' && (
                <div className="p-3 bg-gray-50 rounded-lg">
                  ...
                  <div className="flex items-center gap-1 text-sm text-gray-500 mt-2">
                    ...
                    <span>Last scanned: {selectedLocation.lastScanned}</span>
                  </div>
                </div>
              )}
              
              {selectedLocation.type === 'shop' && selectedLocation.rating && (
                <div className="p-3 bg-gray-50 rounded-lg">
