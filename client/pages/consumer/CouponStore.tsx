    ...
    <div className="min-h-screen bg-gradient-to-b from-emerald-50 via-green-50 to-white pb-20">
      <div className="bg-white border-b border-emerald-100 shadow-sm">
        <div className="max-w-5xl mx-auto px-4 py-4 flex items-center gap-3">
          <Button variant="ghost" size="sm" onClick={() => safeNavigateBack(navigate, "/consumer/home")} className="p-2 rounded-full hover:bg-emerald-100 transition-colors">
      ...
      <div className="bg-gradient-to-r from-emerald-600 to-green-600 text-white py-4 px-6 shadow-md">
        <div className="max-w-5xl mx-auto flex justify-between items-center">
          ...
          <Button onClick={() => navigate("/consumer/wallet")} variant="outline" className="bg-white text-emerald-700 hover:bg-emerald-50 border-0">
      ...
      <Dialog open={showSuccessModal} onOpenChange={setShowSuccessModal}><DialogContent className="bg-white rounded-2xl max-w-sm border-0 shadow-2xl"><div className="absolute -top-12 left-1/2 -translate-x-1/2 bg-emerald-100 rounded-full p-4 border-4 border-white"><CheckCircle className="w-10 h-10 text-emerald-600" /></div><DialogHeader className="text-center pt-6"><DialogTitle className="text-xl font-bold text-emerald-800 mt-4">Coupon Redeemed!</DialogTitle><DialogDescription className="text-emerald-600">{redeemedCoupon && `"${redeemedCoupon.product_name}" added to your wallet.`}</DialogDescription></DialogHeader>{redeemedCoupon && <div className="bg-gray-50 p-3 rounded-lg my-2"><div className="flex justify-between text-sm"><span className="text-gray-500">Coupon value:</span><span className="font-medium">₹{redeemedCoupon.discounted_price}</span></div><div className="flex justify-between text-sm mt-1"><span className="text-gray-500">Points spent:</span><span className="font-medium">-{redeemedCoupon.points_cost} points</span></div><div className="flex justify-between text-sm mt-1"><span className="text-gray-500">Remaining points:</span><span className="font-medium">{userPoints} points</span></div></div>}<DialogFooter className="flex flex-col gap-2 mt-4"><Button onClick={() => navigate("/consumer/wallet")} className="w-full bg-gradient-to-r from-emerald-600 to-green-600 hover:opacity-90"><Wallet className="w-4 h-4 mr-2" /> See in Wallet</Button><Button variant="outline" onClick={() => setShowSuccessModal(false)} className="w-full">Continue Shopping</Button></DialogFooter></DialogContent></Dialog>
      <Dialog open={showInsufficientPointsModal} onOpenChange={setShowInsufficientPointsModal}><DialogContent className="bg-white rounded-2xl max-w-sm border-0 shadow-2xl"><div className="absolute -top-12 left-1/2 -translate-x-1/2 bg-amber-100 rounded-full p-4 border-4 border-white"><AlertCircle className="w-10 h-10 text-amber-600" /></div><DialogHeader className="text-center pt-6"><DialogTitle className="text-xl font-bold text-amber-800 mt-4">Insufficient Points</DialogTitle><DialogDescription className="text-amber-600">You need {selectedCoupon && userPoints !== null ? selectedCoupon.points_cost - userPoints : 0} more points to redeem this coupon.</DialogDescription></DialogHeader>{selectedCoupon && <div className="bg-amber-50 p-3 rounded-lg my-2"><div className="flex justify-between text-sm"><span className="text-amber-700">Coupon:</span><span className="font-medium">{selectedCoupon.product_name}</span></div><div className="flex justify-between text-sm mt-1"><span className="text-amber-700">Points needed:</span><span className="font-medium">{selectedCoupon.points_cost}</span></div><div className="flex justify-between text-sm mt-1"><span className="text-amber-700">Your points:</span><span className="font-medium">{userPoints}</span></div></div>}<DialogFooter className="flex flex-col gap-2 mt-4"><Button onClick={() => { setShowInsufficientPointsModal(false); navigate("/consumer/watch-ads"); }} className="w-full bg-gradient-to-r from-amber-500 to-orange-500 hover:opacity-90 text-white"><Video className="w-4 h-4 mr-2" /> Watch Ads to Earn Points</Button><Button variant="outline" onClick={() => setShowInsufficientPointsModal(false)} className="w-full text-amber-700 border-amber-200 hover:bg-amber-50">Maybe Later</Button></DialogFooter></DialogContent></Dialog>
    </div>
  );
}
